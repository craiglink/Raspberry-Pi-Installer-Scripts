...
dtoverlay=pitft28-capacitive,{rotation}""",
    ...
    },
    {
        ...
        "overlay_dest": "/boot/firmware/overlays/drm-minipitft13.dtbo",
    ...
    },
    {
        ...
        "overlay_dest": "/boot/firmware/overlays/drm-minipitft114.dtbo",
...
def softwareinstall():
    ...
    if not shell.run_command("apt-get install -y bc fbi git python3-dev python3-pip python3-smbus evtest libts-bin device-tree-compiler libraspberrypi-bin libraspberrypi-dev"):
        warn_exit("Apt failed to install software!")
    if not shell.run_command("pip3 install evdev spidev"):
        warn_exit("Pip failed to install software!")
    return True

def uninstall_bootconfigtxt():
    """Remove any old flexfb/fbtft stuff"""
    if shell.pattern_search("/boot/firmware/config.txt", "adafruit-pitft-helper"):
        print("Already have an adafruit-pitft-helper section in /boot/firmware/config.txt.")
        print("Removing old section...")
        shell.run_command("cp /boot/firmware/config.txt /boot/firmware/configtxt.bak")
        shell.pattern_replace("/boot/firmware/config.txt", '\n# --- added by adafruit-pitft-helper.*?\n# --- end adafruit-pitft-helper.*?\n', multi_line=True)
    return True
...
def update_configtxt(rotation_override=None):
    """update /boot/firmware/config.txt with appropriate values"""
    ...
    shell.write_text_file("/boot/firmware/config.txt", """
...
def install_console():
    ...
    if not shell.pattern_search("/boot/firmware/cmdline.txt", 'fbcon=map:10 fbcon=font:VGA8x8'):
        print("Updating /boot/firmware/cmdline.txt")
        shell.pattern_replace("/boot/firmware/cmdline.txt", "rootwait", "rootwait fbcon=map:10 fbcon=font:VGA8x8")
    else:
        print("/boot/firmware/cmdline.txt already updated")
...
def uninstall_console():
    print("Removing console fbcon map from /boot/firmware/cmdline.txt")
    shell.pattern_replace("/boot/firmware/cmdline.txt", 'rootwait fbcon=map:10 fbcon=font:VGA8x8', 'rootwait')
    print("Screen blanking time reset to 10 minutes")
    if shell.exists("/etc/kbd/config"):
        shell.pattern_replace("/boot/firmware/cmdline.txt", 'BLANK_TIME=0', 'BLANK_TIME=10')
    shell.pattern_replace("/etc/rc.local", '^# disable console blanking.*')
...
def install_fbcp():
    ...
    print("Configuring boot/firmware/config.txt for forced HDMI")
    shell.reconfig("/boot/firmware/config.txt", "^.*hdmi_force_hotplug.*$", "hdmi_force_hotplug=1")
    shell.reconfig("/boot/firmware/config.txt", "^.*hdmi_group.*$", "hdmi_group=2")
    shell.reconfig("/boot/firmware/config.txt", "^.*hdmi_mode.*$", "hdmi_mode=87")
    shell.pattern_replace("/boot/firmware/config.txt", "^[^#]*dtoverlay=vc4-fkms-v3d.*$", "#dtoverlay=vc4-fkms-v3d")
    ...
    shell.reconfig("/boot/firmware/config.txt", "^.*hdmi_cvt.*$", "hdmi_cvt={} {} 60 1 0 0 0".format(WIDTH, HEIGHT))
    ...
    if fbcp_rotations[pitftrot] == "0":
        ...
        shell.reconfig("/boot/firmware/config.txt", "^.*display_hdmi_rotate.*$", "")
    else:
        display_rotate = fbcp_rotations[pitftrot]
        shell.reconfig("/boot/firmware/config.txt", "^.*display_hdmi_rotate.*$", "display_hdmi_rotate={}".format(display_rotate))
        # Because we rotate HDMI we have to 'unrotate' the TFT by overriding pitftrot!
        if not update_configtxt(default_orientation):
            shell.bail("Unable to update /boot/firmware/config.txt")
    return True
...
def uninstall_fbcp():
    ...
    print("Configuring boot/firmware/config.txt for default HDMI")
    shell.reconfig("/boot/firmware/config.txt", "^.*hdmi_force_hotplug.*$", "hdmi_force_hotplug=0")
    shell.pattern_replace("/boot/firmware/config.txt", "^.*#.*dtoverlay=vc4-fkms-v3d.*$", "dtoverlay=vc4-fkms-v3d")
    shell.pattern_replace("/boot/firmware/config.txt", '^hdmi_group=2.*$')
    shell.pattern_replace("/boot/firmware/config.txt", '^hdmi_mode=87.*$')
    shell.pattern_replace("/boot/firmware/config.txt", '^hdmi_cvt=.*$')
    return True
...
@click.option('-u', '--user', nargs=1, default="/home/ubuntu", type=str, help="Specify path of primary user's home directory", show_default=True)
...
restart the script and choose a different orientation.""".format(rotation=pitftrot, display=pitft_config["menulabel"]))
    ...
    shell.info("Updating /boot/firmware/config.txt...")
